...
Usage:
    ...
    # Only check whether the databases are outdated (no rebuild):
    python tools/build_database.py --check <url>

How to get the URL (no DevTools needed):
    1. Create a bookmark with this URL (a "bookmarklet"):

       javascript:(()=>{const u=performance.getEntriesByType('resource').map(r=>r.name).find(n=>n.includes('city_entities'));if(u){prompt('Current city_entities metadata URL — copy it:',u)}else{alert('Not found. Click this right after the game finishes loading.')}})()

    2. Open Forge of Empires, wait for the city to load, click the bookmark.
    3. Copy the URL from the prompt and pass it to this script.

    (Alternative: DevTools (F12) -> Network tab -> reload -> filter
     "city_entities" -> right-click request -> Copy URL.)

Output:
    data/foe_buildings_database.js   (main city buildings)
    data/qi_buildings_database.js    (Quantum Incursion / Guild Raids buildings)
    data/db_meta.js                  (generation date + source hash, shown in-app)
...
Note:
    ...
    The hash in the metadata URL changes with every game data update, so
    --check can tell whether your databases are stale without rebuilding.
"""
...
def source_hash(source):
    """Extract the version hash from a metadata URL (id=city_entities-<hash>)."""
    m = re.search(r'city_entities-([0-9a-f]+)', source)
    return m.group(1) if m else None


def read_stored_hash(data_dir):
    """Read sourceHash from an existing data/db_meta.js, or None."""
    meta_path = data_dir / 'db_meta.js'
    if not meta_path.exists():
        return None
    m = re.search(r"sourceHash:\s*'([^']*)'", meta_path.read_text(encoding='utf-8'))
    return m.group(1) if m else None


def write_meta(data_dir, src_hash, main_count, qi_count):
    """Write data/db_meta.js so the app can display database freshness."""
    ...
    meta_path.write_text(
        ...
        f"    sourceHash:  '{src_hash or 'local-file'}',\n"
        f'    mainCount:   {main_count},\n'
        f'    qiCount:     {qi_count},\n'
        '};\n',
        encoding='utf-8')
...
def main():
    ...
    if check_only:
        ...
        if not new_hash:
            print('--check needs a metadata URL containing city_entities-<hash>.')
            sys.exit(2)
        if new_hash == stored_hash:
            print(f'Databases are up to date (hash {new_hash}).')
            sys.exit(0)
        print(f'Databases are OUTDATED: stored hash {stored_hash or "(none)"}, '
              f'current game hash {new_hash}.')
        print('Re-run without --check to rebuild.')
        sys.exit(1)
    ...
    write_meta(data_dir, source_hash(source), len(buildings), len(qi_buildings))
